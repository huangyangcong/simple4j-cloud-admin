...
public class TenantServiceImpl implements ITenantService {
  ...
  public TenantDetailResponse getByTenantId(String tenantId) {
    return tenantMapStruct.toVo(
        tenantMapper.getOne(Wrappers.<Tenant>query().lambda().eq(Tenant::getTenantId, tenantId)));
  }
  ...
  public List<TenantDetailResponse> list(TenantListRequest tenantListRequest) {
    LambdaQueryWrapper<Tenant> queryWrapper =
        Wrappers.<Tenant>lambdaQuery()
            .eq(
                StrUtil.isNotEmpty(tenantListRequest.getTenantId()),
                Tenant::getTenantId,
                tenantListRequest.getTenantId())
            .eq(
                StrUtil.isNotEmpty(tenantListRequest.getContactNumber()),
                Tenant::getContactNumber,
                tenantListRequest.getContactNumber())
            .eq(
                StrUtil.isNotEmpty(tenantListRequest.getTenantName()),
    ...
    List<Tenant> list =
        tenantMapper.list(
            !CommonConstant.ADMIN_TENANT_ID.equals(SecurityUtils.getCurrentTenantId())
                ? queryWrapper.eq(Tenant::getTenantId, securityScope.getTenantId())
                : queryWrapper);
  ...
  public Page<TenantDetailResponse> page(TenantPageRequest tenantPageRequest) {
    LambdaQueryWrapper<Tenant> queryWrapper =
        Wrappers.<Tenant>lambdaQuery()
            .eq(
                StrUtil.isNotEmpty(tenantPageRequest.getTenantId()),
                Tenant::getTenantId,
                tenantPageRequest.getTenantId())
            .eq(
                StrUtil.isNotEmpty(tenantPageRequest.getContactNumber()),
                Tenant::getContactNumber,
                tenantPageRequest.getContactNumber())
            .eq(
                StrUtil.isNotEmpty(tenantPageRequest.getTenantName()),
                Tenant::getTenantName,
    ...
    if (ObjectUtil.isEmpty(tenant.getId())) {
      List<Tenant> tenants =
          tenantMapper.selectList(
              Wrappers.<Tenant>query()
                  .lambda()
                  .eq(Tenant::getIsDeleted, CommonConstant.DB_NOT_DELETED));
