...
public class RoleServiceImpl implements IRoleService {
  ...
  @Override
  public RoleDetailResponse detail(RoleDetailRequest roleDetailRequest) {
    Role detail =
        roleMapper.getOne(Wrappers.<Role>lambdaQuery().eq(Role::getId, roleDetailRequest.getId()));
    ...
    IPage<Role> page =
        roleMapper.page(
            new com.baomidou.mybatisplus.extension.plugins.pagination.Page<>(
                rolePageRequest.getPageNo(), rolePageRequest.getPageSize()),
            queryWrapper);
    Page<Role> pages =
  ...
  public List<RoleDetailResponse> tree(String tenantId) {
    ...
    return TreeUtil.buildTree(
        roleMapStruct.toVo(
            roleMapper.tree(StrUtil.nullToDefault(tenantId, securityScope.getTenantId()), null)));
  }
  ...
  public List<RoleDetailResponse> list(RoleListRequest roleListRequest) {
    LambdaQueryWrapper<Role> queryWrapper =
        Wrappers.<Role>lambdaQuery()
            .eq(
                StrUtil.isNotEmpty(roleListRequest.getRoleName()),
                Role::getRoleName,
                roleListRequest.getRoleName())
            .eq(
                StrUtil.isNotEmpty(roleListRequest.getRoleAlias()),
                Role::getRoleAlias,
    ...
    List<Role> pages =
        roleMapper.list(
            CommonConstant.ADMIN_TENANT_ID.equals(SecurityUtils.getCurrentTenantId())
                ? queryWrapper.eq(Role::getTenantId, securityScope.getTenantId())
                : queryWrapper);
    return roleMapStruct.toVo(pages);
  }
  ...
  public Set<String> getRoleIds(String tenantId, List<String> roleNames) {
    ...
    if (CollUtil.isNotEmpty(roleList)) {
      return roleList.stream().map(Role::getId).collect(Collectors.toSet());
    }
    return null;
  }
