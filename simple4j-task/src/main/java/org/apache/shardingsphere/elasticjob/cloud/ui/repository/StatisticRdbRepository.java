...
public class StatisticRdbRepository {
  ...
  public boolean add(final TaskResultStatistics taskResultStatistics) {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      preparedStatement.setTimestamp(
          3, new Timestamp(taskResultStatistics.getStatisticsTime().getTime()));
      preparedStatement.setTimestamp(
          4, new Timestamp(taskResultStatistics.getCreationTime().getTime()));
  ...
  public boolean add(final TaskRunningStatistics taskRunningStatistics) {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      preparedStatement.setTimestamp(
          3, new Timestamp(taskRunningStatistics.getCreationTime().getTime()));
      preparedStatement.execute();
      result = true;
    } catch (final SQLException ex) {
      // TODO log failure directly to output log, consider to be configurable in the future
  ...
  public boolean add(final JobRunningStatistics jobRunningStatistics) {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      preparedStatement.setTimestamp(
          3, new Timestamp(jobRunningStatistics.getCreationTime().getTime()));
      preparedStatement.execute();
      result = true;
    } catch (final SQLException ex) {
      // TODO log failure directly to output log, consider to be configurable in the future
  ...
  public boolean add(final JobRegisterStatistics jobRegisterStatistics) {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      preparedStatement.setTimestamp(
          3, new Timestamp(jobRegisterStatistics.getCreationTime().getTime()));
      preparedStatement.execute();
      result = true;
    } catch (final SQLException ex) {
      // TODO log failure directly to output log, consider to be configurable in the future
   ...
   */
  public List<TaskResultStatistics> findTaskResultStatistics(
      final Date from, final StatisticInterval statisticInterval) {
   ...
   */
  public TaskResultStatistics getSummedTaskResultStatistics(
      final Date from, final StatisticInterval statisticInterval) {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      while (resultSet.next()) {
        result =
            new TaskResultStatistics(
                resultSet.getInt(1), resultSet.getInt(2), statisticInterval, new Date());
      }
    } catch (final SQLException ex) {
      // TODO log failure directly to output log, consider to be configurable in the future
   ...
   * @return task result statistics
   */
  public Optional<TaskResultStatistics> findLatestTaskResultStatistics(
      final StatisticInterval statisticInterval) {
