...
public final class StatisticManager {
  ...
  @Autowired private StatisticRdbRepository rdbRepository;
  ...
  public TaskResultStatistics getTaskResultStatisticsWeekly() {
    ...
    return rdbRepository.getSummedTaskResultStatistics(
        StatisticTimeUtils.getStatisticTime(StatisticInterval.DAY, -7), StatisticInterval.DAY);
  }
   ...
   * @return task result statistic
   */
  public TaskResultStatistics findLatestTaskResultStatistics(
      final StatisticInterval statisticInterval) {
    if (isRdbConfigured()) {
      Optional<TaskResultStatistics> result =
          rdbRepository.findLatestTaskResultStatistics(statisticInterval);
      if (result.isPresent()) {
        return result.get();
      }
    }
  ...
  public List<TaskResultStatistics> findTaskResultStatisticsDaily() {
    ...
    return rdbRepository.findTaskResultStatistics(
        StatisticTimeUtils.getStatisticTime(StatisticInterval.HOUR, -24), StatisticInterval.MINUTE);
  }
  ...
  public List<TaskRunningStatistics> findTaskRunningStatisticsWeekly() {
    ...
    return rdbRepository.findTaskRunningStatistics(
        StatisticTimeUtils.getStatisticTime(StatisticInterval.DAY, -7));
  }
  ...
  public List<JobRunningStatistics> findJobRunningStatisticsWeekly() {
    ...
    return rdbRepository.findJobRunningStatistics(
        StatisticTimeUtils.getStatisticTime(StatisticInterval.DAY, -7));
  }
