...
public class ConfigBuilder {
  ...
  /** SQL语句类型 */
  @Deprecated private IDbQuery dbQuery;

  @Deprecated private DbType dbType;
  ...
  /** 是否支持注释 */
  @Deprecated private boolean commentSupported;
  ...
  private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
    ...
    if (null != configPathInfo) {
      ...
      setPathInfo(
          pathInfo, template.getService(), outputDir, ConstVal.SERVICE_PATH, ConstVal.SERVICE);
      setPathInfo(
          pathInfo,
        ...
        while (results.next()) {
          ...
          if (keyWordsHandler != null) {
            if (keyWordsHandler.isKeyWords(columnName)) {
              System.err.println(
                  String.format("当前表[%s]存在字段[%s]为数据库关键字或保留字!", tableName, columnName));
